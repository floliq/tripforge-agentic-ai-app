...
CLI-ассистент для планирования поездки: достаточно **короткого** запроса (например, «Рим в этом месяце» или «выходные к друзьям») — ассистент сам уточнит детали в диалоге, затем соберёт факты из туристических API, RAG на ChromaDB, маршрут по дням и смету. Оркестрация — **LangGraph** + **Deep Agents**, LLM — локальная **Ollama**, travel-tools также через **MCP**, на критических шагах — **Human-in-the-Loop**. Подробности — [TZ.md](TZ.md).
...
Во время CLI-сессии вопрос про POI (достопримечательности/места) задаёт сам ИИ-агент в диалоге:
- если ответ **да** — может вызывать `search_poi` / `get_poi_details`;
- если ответ **нет** — строит маршрут без POI-поиска и использует weather-only ветку.

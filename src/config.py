...
class Settings(BaseSettings):
    ...
    ollama_base_url: str = os.getenv(
        "TRIPFORGE_OLLAMA_BASE_URL", "http://localhost:11434"
    )
    ollama_model: str = os.getenv("TRIPFORGE_OLLAMA_MODEL", "qwen3.6:35b")
    ollama_embed_model: str = os.getenv(
        "TRIPFORGE_OLLAMA_EMBED_MODEL", "nomic-embed-text"
    )
    chroma_dir: Path = Path("./data/chroma")
    artifacts_dir: Path = Path("./data/artifacts")
    opentripmap_api_key: str | None = os.getenv("TRIPFORGE_OPENTRIPMAP_API_KEY", None)
    openrouteservice_api_key: str | None = os.getenv(
        "TRIPFORGE_OPENROUTESERVICE_API_KEY", None
    )
